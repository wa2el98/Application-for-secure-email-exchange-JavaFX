...
public class ECCEG {
    ...
    public PairP<Point, Point> encrypt(Point p) {
        ...
        return new PairP<Point, Point>(left, right);
    }

    public List<PairP<Point, Point>> encryptBytes(byte[] bytes) {
        List<PairP<Point, Point>> ret = new ArrayList<>();
        for (int i = 0; i < bytes.length; ++i)
            ret.add(encrypt(ECC.intToPoint(BigInteger.valueOf(bytes[i]))));
        return ret;
    }

    public Point decrypt(PairP<Point, Point> p) {
        Point m = ECC.multiply(privateKey, p.left);
        return ECC.subtract(p.right, m);
    }

    public List<Point> decryptBytes(List<PairP<Point, Point>> l) {
        List<Point> ret = new ArrayList<>();
        for (PairP<Point, Point> p: l)
            ret.add(decrypt(p));
        return ret;
    }
}
